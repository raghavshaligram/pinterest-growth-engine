          ...
          {providers.map((p) => {
            ...
            return (
              <div
                ...
                style={{
                  // Connected providers get a plain/neutral card -- the
                  // green status dot below already communicates "good."
                  // Needing attention (errored, or never configured) is
                  // the state that should draw the eye, so that's what
                  // gets the tinted background now. Previously this was
                  // backwards: `ok` triggered --gradient-primary-soft,
                  // which is a tint of --accent (#C23B22, a brick red) --
                  // so "connected" cards looked warning-tinted while
                  // "not configured" cards looked plain.
